...
export const renderModal = ({
	...
	const modal = creatElement('div', {
		className: 'modal',
	});

	const modalMain = creatElement('div', {
		className: 'modal__main',
	});

	const modalTitle = creatElement('h2', {
		className: 'modal__title',
		textContent: title,
	});

	const modalDescription = creatElement('p', {
		className: 'modal__description',
		textContent: discription,
	});

	const modalForm = creatElement('form', {
		className: 'modal__form',
	});

	modalForm.addEventListener('submit', async (e) => {
		e.preventDefault();
		const itsOk = await submintHandler(e);
		if (itsOk) {
			modal.remove();
		}
	});
	const modalField = creatElement('fieldset', {
		className: 'modal__field',

	})

	const modalLabelLogin = creatElement('label', {
		className: 'modal__label',
	})

	const modalInputLogin = creatElement('input', {
	...
	const modalLabelPassword = creatElement('label', {
		className: 'modal__label',
	})

	const modalInputPassword = creatElement('input', {
		className: 'modal__input',
		type: 'password',
		name: 'password',
		placeholder: 'Пароль',
	...
	const modalSubmintBtn = creatElement('button', {
		className: 'modal__btn btn',
		textContent: btnSubmit,
	})

	const modalCloseBtn = creatElement('button', {
		className: 'modal__close',
		innerHTML: `<svg width="54" height="54" viewBox="0 0 54 54" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	modalForm.append(modalField, modalSubmintBtn);
